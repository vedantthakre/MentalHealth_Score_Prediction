(() => {
  ...
  function bandFor(score) {
    if (score < 4) {
      return {
        key: "low",
        label: "Feeling stretched thin",
        context: "Things look strained right now. A bit more sleep or a little less scrolling could help more than you'd expect.",
      };
    }
    if (score < 7) {
      return {
        key: "mid",
        label: "Steady, mostly balanced",
        context: "Your rhythm looks fairly solid, with some room to rest and recharge.",
      };
    }
    return {
      key: "high",
      label: "Riding high",
      context: "Your habits point to a strong, well-supported baseline. Keep doing what's working.",
    };
  }
  ...
  function renderResult(score) {
    ...
    resultFace.setAttribute("data-band", key);

    gaugeFill.style.width = "0%";
    requestAnimationFrame(() => {
      gaugeFill.style.width = `${(clamped / 10) * 100}%`;
    });
    ...
    try {
      ...
      if (res.status === 422) {
        ...
        renderError(
          "Check your inputs",
          matched
            ? "A few fields got rejected — details are marked on the form."
